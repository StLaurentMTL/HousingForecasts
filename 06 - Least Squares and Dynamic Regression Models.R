...
augment(ARIMA_Housing_Starts) %>% 
  ...
  geom_line(aes(y = Housing_Starts_UT,colour = "Data")) +
  geom_line(aes(y = .fitted,colour = "Fitted")) +
  ggtitle("Housing Starts", "ARIMA regression model fitting") +
  xlab("Date") + ylab("Housing Starts") +
  scale_colour_manual(values=c(Data="black",Fitted="#D55E00")) +
  guides(colour = guide_legend(title = NULL))
...
ARIMA_Housing_Starts %>% 
  ...
  labs(title = "Utah Housing Starts",
       ...
       x = "Date",
       y = "Housing Starts") +
  annotate("text",x = as.Date("2017-01-01"),y = 15000,label = "Housing_Starts_UT ~ CPI + Revolving_Home_Loan_Monthly + SP500_Monthly + 
                                                              Unemployment_Rate_Monthly",
           size = 2,
           hjust = 0)
